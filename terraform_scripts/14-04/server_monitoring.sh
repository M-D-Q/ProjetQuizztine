...
# Fill in the /usr/local/nagios/etc/pg_master.cfg file
sudo bash -c "cat > /usr/local/nagios/etc/pg_master.cfg << EOF
define host {
    use                     linux-server
    host_name               pg_master
...
EOF"
# Fill in the /usr/local/nagios/etc/pg_slave.cfg file
sudo cp /usr/local/nagios/etc/pg_master.cfg /usr/local/nagios/etc/pg_slave.cfg
sudo sed -i 's/pg_master/pg_slave/g' /usr/local/nagios/etc/pg_slave.cfg
sudo sed -i "s/$MASTER_IP/$SLAVE_IP/g" /usr/local/nagios/etc/pg_slave.cfg
...
POSTGRES_EXPORTER_IP="10.0.1.10"
